...
test.describe("Dashboard", () => {
  ...
  test.describe("Deposit Page", () => {
    test("should show wallet address and QR code from the dashboard", async ({ page }) => {
      test.skip(
        !process.env.E2E_TEST_EMAIL,
        "E2E_TEST_EMAIL not set — skipping deposit test"
      );

      await page.goto("/dashboard");
      await page.getByRole("button", { name: "Open deposit modal" }).click();

      await expect(page.getByText("Select a Deposit Method")).toBeVisible({
        timeout: 10000,
      });

      const instantBtn = page
        .locator("button")
        .filter({ hasText: "Instant Deposit" });
      await expect(instantBtn).toBeVisible({ timeout: 5000 });
      await instantBtn.click();

      const dialog = page.getByRole("dialog");
      await expect(dialog).toBeVisible({ timeout: 5000 });

      const qrCode = dialog.locator(
        'svg[aria-label="Wallet address QR code"]'
      );
      await expect(qrCode).toBeVisible({ timeout: 5000 });

      const addressText = await dialog
        .locator("span")
        .filter({ hasText: /^0x/ })
        .textContent();
